...
CREATE FUNCTION JVar(tname TEXT) RETURNS TEXT
  RETURN DECAPITALIZE(JCamelCase(tname));
  
  
DROP FUNCTION IF EXISTS JCamelCase;
CREATE FUNCTION JCamelCase(tname TEXT) RETURNS TEXT
  RETURN (DEL_CHAR_AND_CAPITALIZE(tname,'_'));
  
  
DROP FUNCTION IF EXISTS JGetter;
CREATE FUNCTION JGetter(prop TEXT, type_ TEXT) RETURNS TEXT
  RETURN CONCAT(type_ , ' get',JCamelCase(prop), '();\n');


DROP FUNCTION IF EXISTS JSetter;
DELIMITER //
CREATE FUNCTION JSetter(prop TEXT, type_ TEXT, body BOOL) RETURNS TEXT
BEGIN
  SET @setter = CONCAT('void set',JCamelCase(prop), '(',type_ ,' ', prop,')');
  SET @bodyText = CONCAT('{ this.', prop , ' = ', prop, '; }');
  RETURN CONCAT(@setter, if(body, @bodyText,';'), '\n');
END// DELIMITER ;